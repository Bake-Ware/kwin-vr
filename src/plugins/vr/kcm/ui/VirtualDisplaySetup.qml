...
Item {
    ...
    KCMUtils.SettingStateBinding {
        configObject: kcm.settings
        settingName: "sysInfoEnabled"
    }
    KCMUtils.SettingStateBinding {
        configObject: kcm.settings
        settingName: "sysInfoPositionX"
    }
    KCMUtils.SettingStateBinding {
        configObject: kcm.settings
        settingName: "sysInfoPositionY"
    }
    ...
    ColumnLayout {
        ...
        // System info HUD (MangoHud-style overlay)
        RowLayout {
            Layout.alignment: Qt.AlignHCenter
            spacing: Kirigami.Units.smallSpacing
            ...
            Controls.CheckBox {
                id: tSysInfoEnabled
                text: i18nc("@option:check", "System Info HUD")
                checked: kcm.settings.sysInfoEnabled
                onToggled: kcm.settings.sysInfoEnabled = checked
            }

            Kirigami.ContextualHelpButton {
                toolTipText: xi18nc("@info:tooltip", "Shows a MangoHud-style overlay in VR with FPS, CPU, GPU, and RAM stats. Pinned to camera view.")
            }
        }
        ...
        RowLayout {
            Layout.alignment: Qt.AlignHCenter
            spacing: Kirigami.Units.largeSpacing * 2
            enabled: tSysInfoEnabled.checked

            ColumnLayout {
                spacing: 2
                Controls.Label {
                    Layout.alignment: Qt.AlignHCenter
                    text: i18nc("@label:spinbox", "Sysinfo X")
                    font.bold: true
                }
                RowLayout {
                    Controls.SpinBox {
                        id: tSysInfoPositionX
                        value: Math.round(kcm.settings.sysInfoPositionX * 10)
                        from: -500
                        to: 500
                        editable: true
                        textFromValue: (v, l) => (v / 10).toFixed(1)
            ...
            ColumnLayout {
                ...
                Controls.Label {
                    Layout.alignment: Qt.AlignHCenter
                    text: i18nc("@label:spinbox", "Sysinfo Y")
                    font.bold: true
                }
                RowLayout {
                    Controls.SpinBox {
                        id: tSysInfoPositionY
                        value: Math.round(kcm.settings.sysInfoPositionY * 10)
                        from: -500
                        to: 500
                        editable: true
                        textFromValue: (v, l) => (v / 10).toFixed(1)
    ...
    Binding {
        target: kcm.settings
        property: "sysInfoPositionX"
        value: tSysInfoPositionX.value / 10.0
    }
    Binding {
        target: kcm.settings
        property: "sysInfoPositionY"
        value: tSysInfoPositionY.value / 10.0
    }
}
