...
XrView {
    ...
    xrOrigin: XrOrigin {
        ...
        // Controllers are not used for pointing, draw them just for fun
        Repeater3D {
            model: [
                { controller: XrController.RightController, color: "red" },
                { controller: XrController.LeftController, color: "green" }
            ]
            delegate: XrController {
                controller: modelData.controller
                Model {
                    scale: Qt.vector3d(0.005, 0.005, 0.015)
                    source: "#Cube"
