...
Node {
    ...
    property vector3d innerWindowPosition: {
        const deco = deco3d.kDecoration;
        if(!deco) {
            return Qt.vector3d(0, 0, 0)
        }

        //TODO: This does not account for window geometry (when window has no decorations and has custom geometry set)
        return Qt.vector3d(
                    (deco.borderLeft - deco.borderRight)/2/root.ppu,
                    -(deco.borderTop - deco.borderBottom)/2/root.ppu,
                    0)
    }
