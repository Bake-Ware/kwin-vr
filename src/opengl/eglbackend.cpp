...
void EglBackend::updateDmabufFormats()
{
    ...
    auto filterFormats = [this, &formats, &formatFilter](std::optional<uint32_t> bpc, bool withExternalOnlyYUV) {
    ...
    auto includeShaderConversions = [&formatFilter](QHash<uint32_t, QList<uint64_t>> &&formats) -> QHash<uint32_t, QList<uint64_t>> {
